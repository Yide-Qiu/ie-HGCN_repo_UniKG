...
class ieHGConv(MessagePassing):
    def __init__(self, in_dim, out_dim, num_types, num_relations, n_heads, dropout = 0.2, use_norm = True, use_RTE = True, **kwargs):
        ...
        for t in range(num_types):
            ...
            self.fc_linears.append(nn.Linear(in_dim,   out_dim))
            self.a_linears.append(nn.Linear(out_dim,  out_dim))
            if use_norm:
                self.norms.append(nn.LayerNorm(out_dim))

        self.relation_pri   = nn.Parameter(torch.ones(num_relations, self.n_heads))
        self.relation_att   = nn.Parameter(torch.Tensor(num_relations, self.n_heads, self.d_k, self.d_k))
        self.relation_msg   = nn.Parameter(torch.Tensor(num_relations, self.n_heads, self.d_k, self.d_k))
    ...
    def message(self, edge_index_i, node_inp_i, node_inp_j, node_type_i, node_type_j, edge_type, edge_time):
        ...
        res_msg     = torch.zeros(data_size, 1, self.d_k).to(node_inp_i.device)
        
        for source_type in range(self.num_types):
            sb = (node_type_j == int(source_type))
            fc_linears = self.fc_linears[source_type]
            for target_type in range(self.num_types):
                ...
                for relation_type in range(self.num_relations):
                    ...
                    v_mat = fc_linears(source_node_vec).view(-1, self.n_heads, self.d_k)
...
class GeneralConv(nn.Module):
    def __init__(self, conv_name, in_hid, out_hid, num_types, num_relations, n_heads, dropout, use_norm = True, use_RTE = True):
        ...
        elif self.conv_name == 'ieHGCN':
            self.base_conv = ieHGConv(in_hid, out_hid, num_types, num_relations, n_heads, dropout, use_norm, use_RTE)
